#' Model training
#' @export
#' @rdname model_training
model_training = function(omics_data, response, alphaMin=0, alphaMax=1,
                          alphalength=5, kfolds=5, n_repeats = 1){
  ...
  set.seed(123)
  ctrl$index <- caret::createMultiFolds(response, 5, n_repeats)
  ...
  for (dat in dataset_names) {
    mods[[dat]] <- caret::train(x = omics_data[[dat]], y = response,
              ...
              shiny::tabsetPanel(type = "tabs",
                ...
                shiny::tabPanel("PCA plots",
                  shiny::fluidRow(shiny::column(6,
                    ...
                    shiny::radioButtons(ns("pcaBasePanelRadioButtons"), "Select dataset", dataset_names, inline = TRUE)
                  ),
                    shiny::column(6,
                ...
                shiny::tabPanel("Heatmaps",
                  shiny::fluidRow(shiny::column(6,
                    ...
                    shiny::radioButtons(ns("heatmapBasePanelRadioButtons"), "Select dataset", dataset_names, inline = TRUE)
                  ),
                    shiny::column(6,
                      shiny::h3("Ensemble classifier", align = "center"),
...
#' @export
biomarker_discovery_analysis_server <- function(input, output, session, datasets,
  ...
  # shiny::observeEvent(biomarker_discovery_analysis_ui_vars$build(), {
    ...
    subset_response <- response
    subset_eset <- datasets
    ...
    single <- ensem <- dataset_names <- names(datasets)
    ...
    shiny::withProgress(message = 'Constructing models.',
      detail = 'This may take a while...', value = 0, {
        ...
        perf <- pred %>%
          ...
          dplyr::summarise(auc = as.numeric(pROC::roc(obs~Yes, direction = "<")$auc)) %>%
          ungroup %>%
    ...
    output$pcaBasePanel <- canvasXpress::renderCanvasXpress({
      print(biomarker_discovery_analysis_ui_vars$pcaBasePanelRadioButtons())
    ...
    output$heatmapEnsemblePanel <- canvasXpress::renderCanvasXpress({
      ...
      y[y < -2] <- -2
      y[y > 2] <- 2
      rownames(y) <- paste(rep(names(ensemblePanel), sapply(ensemblePanel, length)),
                            as.character(unlist(ensemblePanel)), sep=".")
    ...
    dat <- mapply(function(x, y) {
      ...
      do.call(cbind, .)
    colnames(dat) <- paste(rep(names(ensemblePanel), sapply(ensemblePanel, length)),
                           as.character(unlist(ensemblePanel)), sep=".")
