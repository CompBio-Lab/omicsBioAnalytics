...
#' @export
#' @rdname de_analysis
de_analysis = function(eset, design, test){
  .list <- list()
  if (any(log2(eset) < 0)){
    ## limma
...
#' @export
dea_ui <- function(id, datasetName, dataset, response, tests) {
  ns <- shiny::NS(id)
  choices <- names(tests)
  names(choices) <- names(tests)
  selected <- choices[1]
...
#' @export
dea_server <- function(input, output, session, datasetName, dataset,
                       response, response_var, perform_pathway_analysis,
                       de_results, group_colors, dea_ui_vars) {
  ns <- session$ns
  ...
  shiny::observeEvent(dea_ui_vars$fdr(), {
    shiny::observeEvent(dea_ui_vars$comparison(), {
      shiny::observeEvent(dea_ui_vars$deTest(), {
        ...
        fit <- de_results[[dea_ui_vars$deTest()]]
        top <- limma::topTable(fit, coef = selectedCoef, adjust.method = "BH",
                               n = nrow(fit), sort.by="none")
        top <- top %>%
          dplyr::mutate(FeatureName = rownames(.), sig = -log10(P.Value)) %>%
